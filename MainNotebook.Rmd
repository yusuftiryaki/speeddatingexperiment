...
plot.score.partners <- ggplot(data = data.score.yourself.partners.all, mapping = aes(x = PositiveResponseScore, color = Gender))  + geom_point() +   geom_smooth(method = lm, se = F)
...
fit <- lm(PositiveResponseScore ~ Attractive.Partners + Ambitious.Partners+ Sincere.Partners+ Intelligent.Partners+ Fun.Partners, data = data.score.yourself.partners.all)
summary(fit)
